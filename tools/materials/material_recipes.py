...
MATERIAL_SPECS: Dict[str, Dict[str, Any]] = {
        ...
        "MAT_Gold_Polished": {
                ...
                "tags": ["metal", "gold"],
                "shader": "principled",
                # Gold base color for metallic workflow (tweak if you want warmer/cooler gold)
...
def _ensure_output_and_principled(nt: bpy.types.NodeTree) -> bpy.types.Node:
        ...
        if "Surface" in out.inputs and "BSDF" in bsdf.outputs:
                have = False
                for l in links:
                        if l.from_node == bsdf and l.to_node == out and l.to_socket == out.inputs[
                                        "Surface"]:
                                have = True
                                break
                if not have:
...
def ensure_principled_material(name: str,
                               spec: Dict[str, Any]) -> bpy.types.Material:
        ...
        _set_input(bsdf, ["Base Color"], (float(base[0]), float(
                base[1]), float(base[2]), float(base[3])))
        ...
        if "specular" in spec:
                ...
                _set_input(bsdf, ["Specular", "Specular IOR Level"],
                           float(spec["specular"]))
        ...
        if "coat" in spec:
                _set_input(bsdf, ["Clearcoat", "Coat Weight"],
                           float(spec["coat"]))

        if "coat_roughness" in spec:
                _set_input(bsdf, ["Clearcoat Roughness", "Coat Roughness"],
...
def create_or_update_all_materials() -> List[bpy.types.Material]:
        ...
        for name, spec in MATERIAL_SPECS.items():
                ...
                if shader != "principled":
                        raise ValueError(
                                f"Unsupported shader type for {name}: {shader}"
                        )
